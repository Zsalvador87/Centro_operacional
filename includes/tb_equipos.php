...
//Variables
$columnas = ['supervisor', 'coordinador', 'gerente'];
$id = 'supervisor';
$tabla = 'plantilla';
...
//Limite
$limite = isset($_POST['registros']) ? mysqli_real_escape_string($db, $_POST['registros']) : 20;
...
$sql = "SELECT SQL_CALC_FOUND_ROWS DISTINCT " . implode(", ", $columnas) ." FROM " . $tabla ." WHERE supervisor NOT IN ('OTRO') AND status NOT IN ('BAJA', 'CAMBIO') " . $where ." ORDER BY supervisor ASC $sLimit;";
//echo $sql;
...
if($num_filas > 0){
    while($row = $resultado->fetch_assoc()){
        ...
        $output['data'] .= '<td>' . $row['coordinador'] . '</td>';
        $output['data'] .= '<td>' . $row['gerente'] . '</td>';
        $output['data'] .= '<td>X</td>';
        $output['data'] .= '<td>X</td>';
        $output['data'] .= '<td>X</td>';
        $output['data'] .= '<td>X</td>';
...
if ($output['total_registros'] > 0) {
    ...
    for($i = $numeroInicio; $i <= $numeroFin; $i++){
        if ($pagina == $i) {
            $output['paginacion'] .= '<li><a class="activo" href="#tbe_tabla">'. $i .'</a></li>';
        } else {
            $output['paginacion'] .= '<li><a href="#tbe_tabla" onclick="obtenerSupervisores('. $i .')">'. $i .'</a></li>';
        }
    }
    $output['paginacion'] .= '</ul>';
